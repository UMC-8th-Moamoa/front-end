...
import api from "../api/axiosInstance"; // baseURL: '/api' 로 설정된 axios 인스턴스 사용
...
function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // ⚠️ 백엔드 로그인 스펙에 맞춰 키 이름을 맞추세요.
      // 예: { user_id, password } 혹은 { username, password } 등
      ...
      if (data.resultType === "SUCCESS") {
        ...
        // 에러 코드를 구분해서 메시지를 다르게 보여주고 싶으면 여기서 분기
  ...
  useEffect(() => {
    // 프리필된 아이디 포커스 UX 등을 원하면 여기서 처리 가능
  }, []);
