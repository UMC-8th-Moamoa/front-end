...
export default function OtherUserFollowListPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (tab === "followings") {
      return base.filter((u) => ["팔로잉", "모아참여중", "모아참여"].includes(u.buttonType));
    }
    return base;
  ...
  const renderButton = (type: FollowType, id: string) => {
    ...
    switch (type) {
      case "모아참여":
        return <button className={`${base} px-[24px] py-[6px] border border-[#6282E1] bg-[#FFF] text-[#6282E1]`}>모아 참여</button>;
      case "맞팔로우":
        return <button onClick={() => handleFollowBack(id)} className={`${base} px-[24px] py-[6px] bg-[#6282E1] !text-[#FFF]`}>맞팔로우</button>;
      case "모아참여중":
        return <button disabled className={`${base} px-[10px] py-[8px] bg-transparent text-[#6282E1] border border-transparent`}>모아 참여 중</button>;
      case "팔로잉":
        return <button onClick={() => handleUnfollowClick(id)} className={`${base} px-[24px] py-[6px] border border-[#C7D5FF] bg-[#FFF] text-[#C7D5FF]`}>팔로잉</button>;
    }
  };
