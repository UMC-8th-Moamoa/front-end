...
export default function OtherUserWishlistPage() {
  ...
  // 라우팅 시 state로 상대 userId / userName을 넘겼다고 가정
  const userId = (location.state as any)?.userId as string | undefined;
  const userNameFromState = (location.state as any)?.userName as string | undefined;

  const [titleName, setTitleName] = useState(userNameFromState ?? "사용자");
  const [loading, setLoading] = useState(true);
  const [isFollowing, setIsFollowing] = useState<boolean>(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  // TODO: 실제 위시리스트 API 나오면 교체
    ...
    async function load() {
      ...
      if (!cancelled) {
        if (res.ok && res.payload?.OtherInfo) {
          ...
          setIsFollowing(Boolean(other.is_following)); // 내가 그를 팔로우 중이면 열람 가능
          ...
          {isFollowing ? (
            <div className="flex flex-col gap-[14px]">
              {wishlist.map((item) => (
                ...
                >
                  <img src={Perfume} alt={item.name} className="w-[105px] h-[105px] rounded-[10px] object-cover bg-[#D9D9D9]" />
                  <div className="flex flex-col justify-center">
                    <span className="text-[14px] py-[12px] mb-[28px] font-medium text-[#1F1F1F]">{item.name}</span>
                    <span className="text-[20px] font-bold text-[#1F1F1F] mt-[6px]" style={{ fontWeight: 600 }}>{item.price}</span>
                  </div>
                </div>
              ))}
            </div>
